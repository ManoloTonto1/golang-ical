...
func ParseProperty(contentLine ContentLine) (*BaseProperty, error) {
	...
	for {
		...
		case ';':
			...
			if err != nil {
				return nil, fmt.Errorf("%s %s: %w", ErrParsingProperty, t, err)
			}
			if r == nil {
				return nil, nil
			}
...
func parsePropertyParam(r *BaseProperty, contentLine string, p int) (*BaseProperty, int, error) {
	...
	case '=':
		p += 1
	default:
		return nil, p, fmt.Errorf("%s for %s in %s", ErrMissingPropertyValue, k, r.IANAToken)
	}
	for {
		if p >= len(contentLine) {
			return nil, p, nil
		...
		if err != nil {
			return nil, 0, fmt.Errorf("%s: %w %s in %s", ErrParse, err, k, r.IANAToken)
		}
